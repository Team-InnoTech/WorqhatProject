...
export const register = async (req: Request, res: Response) => {
    ...
    const userCheck = await worqClient(`select * from users where email = '${email}`);
...
export const login = async (req: Request, res: Response) => {
    ...
    const token = jwt.sign({ id: user.id, email: user.email }, JWT_SECRET as string, { expiresIn: '1d' });
